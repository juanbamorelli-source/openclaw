...
export function buildSkillWorkshopPromptSection(): string[] {
  return [
    ...
    "Use `action=create` for a new skill, `action=update` for an existing approved/live skill, and `action=revise` for an existing pending proposal; keep `description` under 160 bytes and `proposal_content` full replacement SKILL.md content within the configured body limit.",
